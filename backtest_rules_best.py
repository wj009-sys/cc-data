...
# -*- coding: utf-8 -*-
"""
最优组合回测：2021-01-01 ~ 2025-12-31
规则：止盈+20%卖半 + 止损-15%清仓观察40日 + 年度±20%再平衡
输出：所有交易流水 + 每年收益统计
"""
...
class Position:
    ...
    def check_stop_profit(self, date):
        ...
        if pnl_pct >= 0.20:
            ...
            if sell_qty > 0:
                return True, '卖半', sell_qty, f'止盈+20%：浮盈+{pnl_pct*100:.1f}% >= 20%，卖出一半'
    ...
    def check_stop_loss(self, date):
        ...
        if pnl_pct <= -0.15:
            return True, '清仓', self.qty, f'止损-15%：浮盈{pnl_pct*100:.1f}% <= -15%，清仓观察40日'
