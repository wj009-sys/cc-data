...
class Position:
    def __init__(self, etf):
        ...
        # 止盈状态
        self.sp_triggered = set()  # 已触发的止盈档位 {'1','2','3'}
        self.sp_ref_qty = 0        # 止盈参考基数（第一档触发时的持仓量）

        # 止损状态
        self.sl_warning = False    # 高波动-5%预警标记
        self.sl_a_triggered = False  # 高波动-10%卖半
        self.sl_b_triggered = False  # 高波动-15%清仓
        self.sl_unified_triggered = False  # 统一止损-15%
    ...
    def check_stop_profit(self, date):
        """检查止盈，返回 (should_execute, action, qty, reason)"""
        ...
        # 第三档：+12%
        if '3' not in self.sp_triggered and pnl_pct >= 0.12:
            return True, '清仓', self.qty, f'止盈第三档：浮盈+{pnl_pct*100:.1f}% >= 12%，清仓止盈'

        # 第二档：+9%
        if '2' not in self.sp_triggered and pnl_pct >= 0.09:
            sell_qty = round_lot(self.qty * 0.30)
            if sell_qty > 0:
                return True, '减仓30%', sell_qty, f'止盈第二档：浮盈+{pnl_pct*100:.1f}% >= 9%，减仓30%'

        # 第一档：+7%
        if '1' not in self.sp_triggered and pnl_pct >= 0.07:
            sell_qty = round_lot(self.qty * 0.20)
            if sell_qty > 0:
                return True, '减仓20%', sell_qty, f'止盈第一档：浮盈+{pnl_pct*100:.1f}% >= 7%，减仓20%'

        return False, '', 0, ''

    def check_stop_loss(self, date):
        """检查止损，返回 (should_execute, action, qty, reason)"""
        if self.qty <= 0 or self.current_price <= 0:
            return False, '', 0, ''

        pnl_pct = (self.current_price - self.cost_price) / self.cost_price if self.cost_price > 0 else 0

        if self.stop_loss_type == 'high_vol':
            # 高波动品种硬止损（科创50/创业板）
            if pnl_pct <= -0.15 and not self.sl_b_triggered:
                return True, '清仓', self.qty, f'止损B：浮盈{pnl_pct*100:.1f}% <= -15%，清仓观察20日'
            if pnl_pct <= -0.10 and not self.sl_a_triggered:
                sell_qty = round_lot(self.qty * 0.50)
                if sell_qty > 0:
                    return True, '卖半', sell_qty, f'止损A：浮盈{pnl_pct*100:.1f}% <= -10%，卖出50%'
            if pnl_pct <= -0.05 and not self.sl_warning:
                # 预警只记录不操作
                self.sl_warning = True
                return True, '预警', 0, f'预警线：浮盈{pnl_pct*100:.1f}% <= -5%，记录台账'

        elif self.stop_loss_type == 'unified':
            # 统一止损-15%
            if pnl_pct <= -0.15 and not self.sl_unified_triggered:
                return True, '清仓', self.qty, f'统一止损：浮盈{pnl_pct*100:.1f}% <= -15%，清仓观察40日'

        return False, '', 0, ''

    def check_reentry(self, date):
        """检查是否可以重新入场"""
        if self.cleared_date is None and not self.sl_unified_triggered:
            return False
...
for idx, date in enumerate(all_dates):
    ...
    for p, action, qty, reason in sorted(sp_candidates, key=lambda x: x[0].pnl_pct, reverse=True):
        ...
        if sold_qty > 0:
            cash += sold_amt
            # 更新止盈状态
            if '3' not in p.sp_triggered and p.pnl_pct >= 0.12:
                p.sp_triggered.add('3')
            elif '2' not in p.sp_triggered and p.pnl_pct >= 0.09:
                p.sp_triggered.add('2')
            elif '1' not in p.sp_triggered and p.pnl_pct >= 0.07:
                p.sp_triggered.add('1')
        ...
        if p.qty <= 0:
            ...
            if p.cleared_date and p.observe_needed > 0:
                ...
                if est_trading_days >= p.observe_needed and p.reentry_allowed:
                    ...
                    p.cleared_date = None
                    p.observe_needed = 0
                    p.sl_unified_triggered = False
                    p.sl_b_triggered = False
                    p.sl_a_triggered = False
                    p.sl_warning = False
    ...
    for p, action, qty, reason in sl_candidates:
        ...
        if sold_qty > 0:
            cash += sold_amt
            # 更新止损状态
            if p.stop_loss_type == 'high_vol':
                if '止损B' in reason:
                    p.sl_b_triggered = True
                    p.cleared_date = date
                    p.observe_needed = 20
                elif '止损A' in reason:
                    p.sl_a_triggered = True
            elif p.stop_loss_type == 'unified':
                if '统一止损' in reason:
                    p.sl_unified_triggered = True
                    p.cleared_date = date
                    p.observe_needed = 40
